...
"""
=============================================================================
PPO Training for Decoder-Only LLMs (Qwen 2.5 / LLaMA / Mistral)
=============================================================================
This script performs Proximal Policy Optimization (PPO) starting from a
pre-trained Supervised Fine-Tuned (SFT) Decoder-Only model.

Features:
  - Autoregressive Online Rollout Generation with Chat-Template system prompts
  - Zero-VRAM Reference Model evaluation via PEFT adapter disabling (model.disable_adapter())
  - Custom Token-Level Value Head for Generalized Advantage Estimation (GAE)
  ...
  - PPO Clipped Surrogate Loss with per-token KL divergence penalty
...
class PromptDataset(Dataset):
    def __init__(self, data_path: str, min_sim: float = 0.70, max_sim: float = 0.98, max_samples: Optional[int] = None):
        ...
        print(f"Loaded {len(self.samples)} valid source prompts for PPO training.")
...
# ==============================================================================
# VALUE HEAD FOR DECODER-ONLY CAUSAL LM
# ==============================================================================
class CausalLMWithValueHead(nn.Module):
    """
    Wraps AutoModelForCausalLM with a dedicated token-level Value Head.
    """
    def __init__(self, base_model: nn.Module, hidden_size: int, dtype: Optional[torch.dtype] = None):
        super(CausalLMWithValueHead, self).__init__()
    ...
    def forward(
        ...
        if return_values:
            ...
            values = self.v_head(hidden_states.to(self.v_head.weight.dtype)).squeeze(-1)  # (batch_size, seq_len)
        return logits, values


# ==============================================================================
# LOG-PROBABILITIES EXTRACTION
# ==============================================================================
def get_action_log_probs(logits: torch.Tensor, input_ids: torch.Tensor, prompt_lens: List[int]) -> Tuple[List[torch.Tensor], List[torch.Tensor]]:
    """
    Extracts action log-probabilities for generated tokens (excluding prompt tokens).
    """
    batch_size = logits.size(0)
    log_probs = F.log_softmax(logits.float()[:, :-1, :], dim=-1)
    target_ids = input_ids[:, 1:]

    gathered_logps = torch.gather(log_probs, dim=-1, index=target_ids.unsqueeze(-1)).squeeze(-1)

    batch_action_logps = []
    batch_masks = []
    for i in range(batch_size):
        p_len = prompt_lens[i] - 1  # Shifted due to :-1
        seq_logps = gathered_logps[i, p_len:]
        batch_action_logps.append(seq_logps)
        batch_masks.append(torch.ones_like(seq_logps, dtype=torch.bool))

    return batch_action_logps, batch_masks
...
# ==============================================================================
def main():
    parser = argparse.ArgumentParser(description="PPO Training for Decoder-Only LLMs (Qwen 2.5).")
    parser.add_argument("--corpus_path", type=str, required=True, help="Path to corpus JSON/CSV for source prompts")
    parser.add_argument("--sft_model_path", type=str, required=True, help="Path to pre-trained SFT model checkpoint")
    ...
    parser.add_argument("--output_dir", type=str, default="results/models/decoder_only/ppo")
    parser.add_argument("--log_dir", type=str, default="results/logs/experiments/ppo/decoder_only")
    parser.add_argument("--plot_dir", type=str, default="results/plots/experiments/ppo/decoder_only")
    ...
    parser.add_argument("--vf_coef", type=float, default=0.5, help="Value loss coefficient in total objective")
    ...
    log_file = os.path.join(args.log_dir, f"train_ppo_decoder_only_{timestamp}.log")
    sys.stdout = Logger(log_file)
    sys.stderr = sys.stdout

    print("=" * 80)
    print(f"Starting Decoder-Only PPO Training at {timestamp}")
    ...
    print(f"Loading Base Causal LM from {args.base_model_name}...")
    ...
    if os.path.exists(os.path.join(args.sft_model_path, "adapter_config.json")):
        ...
        base_lm = AutoModelForCausalLM.from_pretrained(args.base_model_name, torch_dtype=torch_dtype, trust_remote_code=True)
        ...
        base_lm = AutoModelForCausalLM.from_pretrained(args.sft_model_path, torch_dtype=torch_dtype, trust_remote_code=True)
    else:
        print(f"Loading base model {args.base_model_name}...")
        base_lm = AutoModelForCausalLM.from_pretrained(args.base_model_name, torch_dtype=torch_dtype, trust_remote_code=True)

    if args.use_peft:
        print("Initializing LoRA adapters for PPO Policy...")
    ...
    hidden_size = getattr(base_lm.config, "hidden_size", 1536)
    model = CausalLMWithValueHead(base_lm, hidden_size=hidden_size, dtype=torch_dtype).to(device)
    ...
    # ==========================================================================
    global_step = 0
    print("Beginning PPO Rollout & Update Loop...")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_prompts in progress_bar:
            ...
            prompt_lens = [len(ids) for ids in prompt_input_ids]

            # ------------------------------------------------------------------
            # 1. Rollout Generation
            # ------------------------------------------------------------------
            with torch.no_grad():
                gen_outputs = model.pretrained_model.generate(
            ...
            for i in range(len(batch_prompts)):
                gen_tokens = gen_outputs[i, prompt_lens[i]:]
                gen_str = tokenizer.decode(gen_tokens, skip_special_tokens=True).strip()
                generated_texts.append(gen_str)
            ...
            # Forward passes for initial log-probs and reference log-probs
            with torch.no_grad():
                policy_logits, policy_values = model(gen_outputs, return_values=True)
                old_logps, masks = get_action_log_probs(policy_logits, gen_outputs, prompt_lens)

                if hasattr(model.pretrained_model, "disable_adapter"):
                    with model.pretrained_model.disable_adapter():
                        ref_logits, _ = model(gen_outputs, return_values=False)
                else:
                    ref_logits = policy_logits
                ref_logps, _ = get_action_log_probs(ref_logits, gen_outputs, prompt_lens)
            ...
            for i in range(len(batch_prompts)):
                p_len = prompt_lens[i]
                seq_v = policy_values[i, p_len - 1:].float()
                seq_old_lp = old_logps[i]
                seq_ref_lp = ref_logps[i]
                ...
                action_values.append(seq_v[:len(seq_r)])
            ...
            # ------------------------------------------------------------------
            # 4. PPO Optimization Epochs (Mini-Batch Updates)
            # ------------------------------------------------------------------
            model.train()
            for ppo_epoch in range(args.ppo_epochs):
                for i in range(0, len(batch_prompts), args.mini_batch_size):
                    ...
                    mb_gen_outputs = gen_outputs[mb_indices]
                    mb_prompt_lens = [prompt_lens[idx] for idx in mb_indices]

                    cur_logits, cur_values = model(mb_gen_outputs, return_values=True)
                    cur_logps, _ = get_action_log_probs(cur_logits, mb_gen_outputs, mb_prompt_lens)
                    ...
                    for mb_i, orig_i in enumerate(mb_indices):
                        p_len = mb_prompt_lens[mb_i]
                        act_v = cur_values[mb_i, p_len - 1: p_len - 1 + len(returns[orig_i])].float()
                        act_lp = cur_logps[mb_i]
                        act_old_lp = old_logps[orig_i].detach()
    ...
    # ==========================================================================
    print(f"\nSaving merged standalone Decoder-Only PPO model and Value Head to: {args.output_dir}")
    try:
    ...
    history_file = os.path.join(args.output_dir, "ppo_training_history.json")
    ...
    # ==========================================================================
    plot_path = os.path.join(args.plot_dir, "ppo_decoder_only_training_curves.png")
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))

    axes[0, 0].plot(history["step"], history["mean_reward"], marker="o", color="#1f77b4", label="Composite Reward")
    axes[0, 0].set_title("Mean Composite Reward")
    ...
    axes[1, 1].set_title("PPO Loss Curves")
    ...
    plt.close()
    print(f"PPO training curves saved to: {plot_path}")
    print("=" * 80)
