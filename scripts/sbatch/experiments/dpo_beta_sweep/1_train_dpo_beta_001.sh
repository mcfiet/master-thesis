#!/bin/bash
set -e
#SBATCH --job-name=train_dpo_beta_001
...
mkdir -p results/models/dpo_beta_sweep/dpo_beta_001
...
srun python scripts/modeling/train_dpo.py \
    ...
    --eval_file "$EVAL_FILE" \
    --output_dir "results/models/dpo_beta_sweep/dpo_beta_001" \
    --loss_type "sum" \
    --use_peft \
    --lora_r 16 \
    --lora_alpha 32 \
...
echo "=== DPO Training (Beta=0.01) Completed ==="
date
