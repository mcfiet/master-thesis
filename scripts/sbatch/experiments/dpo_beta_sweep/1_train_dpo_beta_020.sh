#!/bin/bash
#SBATCH --job-name=train_dpo_beta_020
...
mkdir -p results/models/dpo_beta_sweep/dpo_beta_020
mkdir -p results/logs/experiments/dpo_beta_sweep results/plots/experiments/dpo_beta_sweep results/evaluation

echo "=== Training DPO Model with Beta = 0.20 (Conservative Regularization) ==="
date
...
srun python scripts/modeling/train_dpo.py \
    ...
    --eval_file "$EVAL_FILE" \
    --output_dir "results/models/dpo_beta_sweep/dpo_beta_020" \
    --loss_type "mean" \
    --use_peft \
    --lora_r 16 \
    --lora_alpha 32 \
...
echo "=== DPO Training (Beta=0.20) Completed ==="
date
