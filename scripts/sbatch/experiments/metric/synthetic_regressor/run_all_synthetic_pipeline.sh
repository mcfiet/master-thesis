...
JOB4=$(sbatch --parsable --dependency=afterok:$JOB3 scripts/sbatch/experiments/metric/synthetic_regressor/4_train_sft_synthetic.sh)
echo "Step 4 (Train SFT Synthetic): Job ID $JOB4"

# Stufe 4: DPO Paare mit Synthetischem Regressor Reward generieren
JOB5=$(sbatch --parsable --dependency=afterok:$JOB3:$JOB4 scripts/sbatch/experiments/metric/synthetic_regressor/5_generate_dpo_synthetic.sh)
echo "Step 5 (Generate DPO Synthetic): Job ID $JOB5"

# Stufe 5: DPO Training
JOB6=$(sbatch --parsable --dependency=afterok:$JOB4:$JOB5 scripts/sbatch/experiments/metric/synthetic_regressor/6_train_dpo_synthetic.sh)
echo "Step 6 (Train DPO Synthetic): Job ID $JOB6"

# Stufe 6: Evaluierung
JOB7=$(sbatch --parsable --dependency=afterok:$JOB3:$JOB6 scripts/sbatch/experiments/metric/synthetic_regressor/7_evaluate_synthetic_experiments.sh)
