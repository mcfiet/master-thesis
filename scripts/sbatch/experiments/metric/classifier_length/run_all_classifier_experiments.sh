...
JOB_256=$(sbatch scripts/sbatch/experiments/metric/classifier_length/1_train_article_classifier_256.sh | awk '{print $NF}')
echo "[SUBMITTED] 1_train_article_classifier_256.sh (JobID: $JOB_256)"

JOB_512=$(sbatch scripts/sbatch/experiments/metric/classifier_length/1_train_article_classifier_512.sh | awk '{print $NF}')
echo "[SUBMITTED] 1_train_article_classifier_512.sh (JobID: $JOB_512)"

JOB_1024=$(sbatch scripts/sbatch/experiments/metric/classifier_length/1_train_article_classifier_1024.sh | awk '{print $NF}')
echo "[SUBMITTED] 1_train_article_classifier_1024.sh (JobID: $JOB_1024)"

JOB_EVAL=$(sbatch --dependency=afterok:$JOB_256,$JOB_512,$JOB_1024 scripts/sbatch/experiments/metric/classifier_length/2_evaluate_classifiers.sh | awk '{print $NF}')
