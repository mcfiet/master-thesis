...
| **05** | `05_build_corpus_master.sh` | GPU (MIG 24GB) | 4h | Filterung, Deduplizierung & Master CSV/JSON Erstellung |
| **06** | `06_prepare_10kgnad_dpo_corpus.sh` | CPU (4 Cores, 16GB) | 1h | Bereinigung des 10kGNAD Alltagssprache-Korpus für DPO |
| **07** | `07_train_sentence_classifier.sh` | GPU (MIG 24GB) | 4h | BiLSTM Satz-Klassifikator trainieren |
| **08** | `08_train_article_classifier.sh` | GPU (MIG 24GB) | 4h | BiLSTM Artikel-Klassifikator trainieren |
| **09** | `09_train_mixup_regressor.sh` | GPU (MIG 24GB) | 4h | BiLSTM MixUp-Regressor Style-Score Modell trainieren |
| **10** | `10_train_sft.sh` | GPU (MIG 24GB) | 12h | mBART-50 SFT Training auf `corpus_master.json` |
| **11** | `11_generate_dpo_dataset.sh` | GPU (MIG 24GB) | 12h | DPO-Paare offline erzeugen auf 10kGNAD (Temperature Ladder) |
| **12** | `12_train_dpo.sh` | GPU (MIG 24GB) | 12h | Seq2Seq LoRA DPO Training (mBART-50) |
| **13** | `13_evaluate_pipeline.sh` | GPU (MIG 24GB) | 2h | Finale Benchmark-Evaluierung auf Lebenshilfe |
...
## 6. Slurm Cluster Monitoring & Troubleshooting

```bash
# Aktuell laufende und wartende Jobs anzeigen:
squeue -u $USER
