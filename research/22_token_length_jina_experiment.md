...
- **BiLSTM Simplicity Regressor**: Skaliert bis 1000 Tokens (`metric_mixup_1000` verarbeitet den gesamten Text).
- **SBERT MPNet**: Besitzt eine harte interne Grenze von **`max_seq_length = 128` Tokens**.
  - Bei der Berechnung des Semantik-Rewards $R_{sem}$ auf 256-, 500- und 1000-Token-Sequenzen wurden Eingabetexte stillschweigend nach 128 Tokens abgeschnitten.
...
| **`sft_len500`** | SFT Baseline | 500 | $\mathbf{0.6583}$ | $0.9011$ | $0.8943$ | $\mathbf{0.7797}$ 🏆 | $0.0198$ | $0.1493$ | $237.6$ | $43.2\,\%$ |
| **`sft_len1000`**| SFT Baseline | 1000 | $0.5795$ | $\mathbf{0.9179}$ | $\mathbf{0.8965}$ | $0.7487$ | $\mathbf{0.0259}$ 🏆 | $\mathbf{0.1615}$ 🏆 | $\mathbf{289.1}$ 🏆 | $18.9\,\%$ |
...
| `dpo_len500` | DPO (MPNet 128) | 500 | $0.4175$ | $0.8758$ | $0.8421$ | $0.6466$ | $0.0009$ | $0.0422$ | $43.2$ | $29.7\,\%$ |
| `dpo_len500_jina` | DPO (Jina Long) | 500 | $0.4328$ | $0.8753$ | $0.8414$ | $0.6541$ | $0.0009$ | $0.0424$ | $43.7$ | $27.0\,\%$ |
| `dpo_len1000` | DPO (MPNet 128) | 1000 | $0.3376$ | $0.8960$ | $0.8519$ | $0.6168$ | $0.0024$ | $0.0651$ | $114.1$ | $10.8\,\%$ |
| **`dpo_len1000_jina`** | DPO (Jina Long) | 1000 | $0.3297$ | $\mathbf{0.9000}$ | $\mathbf{0.8572}$ | $0.6148$ | $\mathbf{0.0034}$ (+39%) | $\mathbf{0.0682}$ | $\mathbf{125.6}$ (+11.5) | $\mathbf{5.4\,\%}$ 🏆 |

---

## 4. Direkter Vergleich: Was hat sich durch Jina verändert?

### 1. Deutlichste Effekte bei 1000 Tokens (`dpo_len1000_jina` vs. `dpo_len1000`):
...
### 2. Moderate Effekte bei 500 Tokens (`dpo_len500_jina` vs. `dpo_len500`):
...
1. **Behebung des Metric-Bottlenecks**: Jina Embeddings ermöglichen eine ganzheitliche DPO-Optimierung über die gesamte Dokumentlänge. Bei 1000 Tokens führt dies zu längeren, besser abgerundeten Texten und halbiert die verbleibende Truncation-Rate von $10.8\,\%$ auf $5.4\,\%$.
2. **SFT bleibt führend bei echter Vereinfachungssyntax**: `sft_len1000` generiert $289.1$ Tokens und erzielt den höchsten BLEU- ($0.0259$) und Simplicity-Wert ($0.5795$), da es echte Leichte-Sprache-Satzstrukturen formuliert. DPO mit $w_{sem} = 0.5$ neigt weiterhin zur Extraktion, profitiert bei 1000 Tokens durch Jina jedoch signifikant an Kohärenz und Vollständigkeit.
...
### 1. Metrik-Modell: **1000 Tokens (`metric_mixup_1000`)** 🏆
* **Empfehlung**: `max_seq_len = 1000`
* **Begründung**:
  - Erzielt **100.0 % Klassifikationsgenauigkeit** ($LS > AS$) und die maximale Trennschärfe ($\Delta = 0.6628$) auf dem Lebenshilfe-Datensatz.
  - Eliminiert den bei 256 Tokens beobachteten Längen-Bias (kurze Einleitungen von Alltagsartikeln führen bei 256 Tokens zu falsch-hohen $\lambda$-Werten; bei 1000 Tokens wird der Gesamtartikel korrekt als schwer erkannt).
  - Das BiLSTM agiert stabil als globaler Stildichte-Akkumulator ohne Gradient-Degradation.

### 2. SFT Fine-Tuning: **1000 Tokens (`sft_len1000`)** 🏆
* **Empfehlung**: `max_source_len = 1000`, `max_target_len = 1000`, `batch_size = 2`, `accumulation_steps = 8`
* **Begründung**:
...
### 3. DPO Alignment: **1000 Tokens mit Jina Long-Context (`dpo_len1000_jina`)** 🏆
* **Empfehlung**: `max_source_len = 1000`, `max_target_len = 1000`, `--sbert_model_name "jinaai/jina-embeddings-v2-base-de"`, `$w_{style} = 0.7, w_{sem} = 0.3$`
* **Begründung**:
...
├────────────────────────────────────────────────────────────────────────────────────────┤
│ 1. Simplicity-Metrik:  BiLSTM MixUp Regressor ──► max_seq_len = 1000                   │
│ 2. SFT Fine-Tuning:    mBART-50 LoRA          ──► max_src = 1000, max_tgt = 1000       │
│ 3. DPO Alignment:      DPO Alignment (Jina)   ──► max_len = 1000, w_style=0.7, w_sem=0.3│
└────────────────────────────────────────────────────────────────────────────────────────┘
```
